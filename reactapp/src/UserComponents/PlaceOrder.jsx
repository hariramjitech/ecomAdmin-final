...
const PlaceOrder = () => {
  ...
  useEffect(() => {
    ...
    if (storedUserId) {
      ...
      setOrder(prev => ({ ...prev, userId: parsedUserId }));
    } else {
  ...
  const handleSubmit = async () => {
    if (!userId) {
      showMessage("❌ User not logged in. Please login first.", 'error');
      return;
    }
    ...
    try {
      ...
      showMessage("✅ Order placed successfully and stock updated!", 'success');
      
      setTimeout(() => {
        setOrder({
          userId: userId, // Keep userId when resetting
          customerName: '',
          customerEmail: '',
          shippingAddress: '',
          orderItems: []
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-1 leading-tight">Place New Order</h1>
              <p className="text-gray-600">Add products and customer details to place an order</p>
              {userId && <p className="text-sm text-blue-600 mt-1">User ID: {userId}</p>}
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
          ...
          {order.orderItems.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Add products from above to place your order</p>
            </div>
          ) : (
